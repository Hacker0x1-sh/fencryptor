...
export function generatePDFReport(data: ReportData, toolDir: string): string {
  ...
  if (data.affectedFiles.length === 0) {
    ...
    data.affectedFiles.forEach(file => {
      doc.text(`• ${file} (${data.operation === 'encrypt' ? 'Encrypted' : 'Decrypted'})`);
    });
  }
  doc.moveDown(1);
  ...
  doc.on('pageAdded', () => {
    ...
    doc.font('Helvetica').fontSize(8).fillColor('#666666')
      .text(`Generated by HKHackerCloud - fencryptor | Page ${doc.bufferedPageRange().start + 1}`, 40, doc.page.height - 50, { align: 'center' });
  });
  doc.moveTo(40, doc.page.height - 60).lineTo(570, doc.page.height - 60).stroke('#D3D3D3');
  doc.fontSize(8).fillColor('#666666')
    .text('Generated by HKHackerCloud - fencryptor | Cybersecurity', 40, doc.page.height - 50, { align: 'center' });
...
- fencryptor-report.pdf: A detailed PDF report with operation summary, affected files list, and logs.
